...
class AoAModel(nn.Layer):
    ...
    def _forward_step(self, it, fc_feats, att_feats, p_att_feats, att_masks, state):
        ...
        output_emb, state = self.core(word_embs, fc_feats, att_feats, p_att_feats, state, att_masks)
        output = self.classifier(output_emb)

        logprobs = nn.functional.log_softmax(output, axis=1)
        return output, logprobs, state, output_emb
    ...
    def forward_xe(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        for i in range(seq.shape[1] - 1):
            ...
            output, logprobs, state, cap_output = self._forward_step(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
        ...
        cap_outputs = paddle.stack(cap_outputs, axis=1) 
        ...
        return logit_outputs, prob_outputs, img_class, cap_class
