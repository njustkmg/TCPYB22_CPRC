...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        # classfication
        # 为什么这几个网络层会出现   AttributeError: 'NoneType' object has no attribute 'data'  的错误
        # 说明是这几个层没有被更新...
        self.avg_pooling = nn.AdaptiveAvgPool2d((1, self.rnn_size))
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        cap_outputs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.rnn_size)
        ...
        for i in range(seq.size(1) - 1):
            if self.training and i >= 1 and self.ss_prob > 0.0 and self.ss_prob != 1.0: # otherwiste no need to sample
                sample_prob = fc_feats.new(batch_size).uniform_(0, 1)   # 这一步就是用来生成随机概率
                sample_mask = sample_prob < self.ss_prob     # 对于概率小于self.ss_prob变为true，否则为False
                if sample_mask.sum() == 0:
                    ...
                    # 这里为什么要取对数？因为outputs之前取了log.
                    prob_prev = torch.exp(outputs[:, i-1].detach()) # fetch prev distribution: shape Nx(M+1)
                    it.index_copy_(0, sample_ind, torch.multinomial(prob_prev, 1).view(-1).index_select(0, sample_ind))
            
            # 对于无监督数据进行文本生成
            elif self.training and i >= 1 and self.ss_prob == 1.0:
                sample_prob = fc_feats.new(batch_size).uniform_(0, 1)   # 这一步就是用来生成概率
                sample_mask = sample_prob < self.ss_prob
                sample_ind = sample_mask.nonzero().view(-1)
                it = seq[:, i].data.clone()  # 其实这一块连句子的长度都是不固定的，暂时先忽略
                #prob_prev = torch.exp(outputs[-1].data.index_select(0, sample_ind)) # fetch prev distribution: shape Nx(M+1)
                #it.index_copy_(0, sample_ind, torch.multinomial(prob_prev, 1).view(-1))
                # prob_prev = torch.exp(outputs[-1].data) # fetch prev distribution: shape Nx(M+1)
                prob_prev = torch.exp(outputs[:, i-1].detach()) # fetch prev distribution: shape Nx(M+1)
            ...
            output, state, cap_output = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            outputs[:, i] = output
            cap_outputs[:, i] = cap_output

        # classfication
        sectence_emb = self.avg_pooling(cap_outputs)
        img_class = self.predict(p_fc_feats)
        cap_class = self.predict(sectence_emb)
        img_class = self.sigmoid(img_class)
        cap_class = self.sigmoid(cap_class)

        return outputs, img_class, cap_class
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state, cap_output = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state)
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state, cap_output = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
