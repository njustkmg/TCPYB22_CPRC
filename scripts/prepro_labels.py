...
def build_vocab(imgs, params):
  ...
  counts = {}
  with open('/home/whc/Desktop/TCYB/AoANet-pytorch/data/dataset_coco.json', 'r') as f:
    data_100 = json.load(f)

  imgs_100 = data_100['images']
  for img in imgs_100:
    for sent in img['sentences']:
      for w in sent['tokens']:
        counts[w] = counts.get(w, 0) + 1
        # get()返回指定键的值，如果键不在字典中返回默认值 None 或者设置的默认值。
...
if __name__ == "__main__":
  ...
  # parser.add_argument('--word_count_threshold', default=100, type=int, help='only words that occur more than this number of times will be put in vocab')
